...
peakDetectionWindows <- function() {
    ...
    graph <- metadata |>
        select(condition_id, scr_peak_detection_window_min, scr_peak_detection_window_max) |>
        drop_na() |>
        distinct() |>
        mutate(diff = scr_peak_detection_window_max - scr_peak_detection_window_min) |>
        ...
        ggplot(aes(x = condition_id, y = value, fill = window, group = window)) +
            ...
            labs(title = "SCR Peak Detection Windows by Study", x = "Study", y = "Time (s)", fill = "Window")
