...
- [x] Slice 2: Centralize cross-cutting utility helpers in `R/utility.R`.
...
- `traceRemovedRows()` remains a diagnostic helper. If it becomes hot in real workflows, the nested reason-building loop is the first candidate for vectorization because it scales by rows times checked aesthetics.
...
measuresHeatmap(fixture_long_data(), fixture_metadata(), fixture_codebook())
Rprof(NULL)
